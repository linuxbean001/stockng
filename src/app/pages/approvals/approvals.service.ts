...
import { Table, SearchResult } from './approvals.model';

import { tableData } from './data';

import { SortDirection } from './approvals-sortable.directive';
...
function matches(tables: Table, term: string, pipe: PipeTransform) {
     return tables.requesttype.toLowerCase().includes(term)
        || tables.purpose.toLowerCase().includes(term)
        || tables.organization.toLowerCase().includes(term)
        || tables.status.toLowerCase().includes(term)
        || tables.priority.toLowerCase().includes(term);
