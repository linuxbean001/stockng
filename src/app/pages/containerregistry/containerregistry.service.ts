...
import { Table, SearchResult } from './containerregistry.model';

import { tableData } from './data';

import { SortDirection } from './containerregistry-sortable.directive';
...
function matches(tables: Table, term: string, pipe: PipeTransform) {
    return tables.reports.toLowerCase().includes(term)
        || tables.date.toLowerCase().includes(term)
        || tables.year.toLowerCase().includes(term)
        || tables.action.toLowerCase().includes(term);
