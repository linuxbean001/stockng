...
import { Table, SearchResult } from './persistent.model';

import { tableData } from './data';

import { SortDirection } from './persistent-sortable.directive';
...
function matches(tables: Table, term: string, pipe: PipeTransform) {
    return tables.name.toLowerCase().includes(term)
        ...
        || tables.accessmodes.toLowerCase().includes(term)
        || tables.reclaimpolicy.toLowerCase().includes(term)
        || tables.status.toLowerCase().includes(term)
        || tables.claim.toLowerCase().includes(term)
        || tables.storageclass.toLowerCase().includes(term)
        || tables.reason.toLowerCase().includes(term)
