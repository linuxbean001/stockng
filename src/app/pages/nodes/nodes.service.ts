...
import { Table, SearchResult } from './nodes.model';

import { tableData } from './data';

import { SortDirection } from './nodes-sortable.directive';
...
function matches(tables: Table, term: string, pipe: PipeTransform) {
    return tables.name.toLowerCase().includes(term)
        ...
        || tables.ready.toLowerCase().includes(term)
        || tables.cpurequests.toLowerCase().includes(term)
        || tables.cpulimits.toLowerCase().includes(term)
        || tables.memoryrequests.toLowerCase().includes(term)
        || tables.memorylimits.toLowerCase().includes(term)
