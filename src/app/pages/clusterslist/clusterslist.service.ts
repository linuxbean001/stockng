...
import { Table, SearchResult } from './clusterslist.model';

import { tableData } from './data';

import { SortDirection } from './clusterslist-sortable.directive';
...
function matches(tables: Table, term: string, pipe: PipeTransform) {
    return tables.cluster.toLowerCase().includes(term)
        || tables.organization.toLowerCase().includes(term)
        || tables.cp.toLowerCase().includes(term)
        || tables.memory.toLowerCase().includes(term)
        || tables.storage.toLowerCase().includes(term)
        || tables.location.toLowerCase().includes(term);
...
export class ClusterslistService {
